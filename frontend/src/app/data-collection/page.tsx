...
const COLLECTION_STEPS = [
  ...
  ['录音中', 'App/浏览器在本机生成音频，同时将当前语音送往实时识别链路，提供可回听的录音和自动参考文字。'],
      ...
      <article className="mx-auto max-w-4xl rounded-3xl border border-stone-200 bg-white px-5 py-7 shadow-sm sm:px-10 sm:py-10">
        ...
        <div className="mt-9 space-y-9 text-pretty text-sm leading-7 text-gray-700">
          ...
          <section>
            ...
            <p className="mt-4">题目目标句是监督样本的主要文本依据；ASR 结果只是参考文字，不会因为识别错误就自动改写为“用户实际说了什么”。自动质量检查只会标记客观收音问题或进入人工复核；是否重录由你回听后决定。这些结果不会自行作出医疗结论，也不会单独批准训练导入。</p>
