...
- [x] 单字题库不再作为顶层专区，归入“音系强化”内部主题；用户看到的顶层名称为“沟通更顺”，现有题库分为“一字一音”“多音字定音”“系统缺口”“声韵声调/连续音系”。
- [x] 顶层只保留“音系强化”，内部数据保留 `字音起步` 来源标签和 `音系强化` 兼容，不增加平行 topic。
- [x] 接入 Web/Mobile catalog、录音 collection plan 与上传题面；单字题目使用 `prompt_type=single_character`、`target`、`pronunciation_hint` 和来源索引，未把单字直接混入自然句覆盖统计。
- [x] 验证：前端单测相关 17 项通过；前端 TypeScript、Mobile TypeScript 通过。尚未部署，未做真实账号录音 smoke。
