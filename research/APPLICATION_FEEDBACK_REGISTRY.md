...
| RF-008 | 声音与沟通表现 | [报告研究](speech-health/VOICE_AND_COMMUNICATION_PERFORMANCE_REPORT_RESEARCH_2026-08-16.md) · [普通话构音表现基线产品化记录](speech-health/MANDARIN_ARTICULATION_BASELINE_PRODUCTIZATION_2026-09-13.md) · Shor 2019 · ASHA · Mou 2018 · ELS/UEP 共识 | 可用固定 50 个单音节观察系统听懂、音系差异和收音，并与自由表达分开；ASR 单字结果不能替代专业听辨，题面授权和成人目标人群适用性仍待复核；禁止自动医学或训练支持分级 | `validate` | Web/App 以“普通话构音表现基线”直接替换旧 20 词实现；删除旧题目、ID、路由、任务和阈值，不兼容旧数据。owner：训练工作台 | 版权与 SLP/语言学复核；Web/App 回归与真实录音；目标用户完成率；人工听辨一致性；同设备复测稳定性；无诊断性文案或持久化医学标签 |
